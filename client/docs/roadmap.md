...
**How to read:** Day numbers are _client-only_ (Day 1 is first client commit after reset, independent of backend days 1–78). Each day ends in a runnable, tested state with `npm run dev -- --open` at `http://localhost:5173` showing the slice and `npm run test:e2e` covering that slice.
...
_This roadmap lists days that **will be** implemented for the SvelteKit client. For what **is** implemented on the backend, see `docs/IMPLEMENTED.md`; for backend roadmap, see `docs/roadmap.md`. Last updated: 2026-08-26 for `client/` SvelteKit Day 1 start with Playwright._
