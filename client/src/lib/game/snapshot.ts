...
export function isValidPublicSnapshot(snap: unknown): snap is PublicSnapshot {
  ...
  if (typeof s.currentSeat !== "number" || typeof s.stockCount !== "number" || typeof s.winner !== "number") return false;
  if (!Array.isArray(s.players) || !Array.isArray(s.discardRow) || !Array.isArray(s.tableMelds)) return false;
  return true;
}
