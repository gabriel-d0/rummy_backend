...
export function onPrivateSnapshot(snap: PrivateSnapshot): void {
  ...
  console.log(`received op 100 PrivateSnapshot v=${snap.v} gamePhase=${snap.gamePhase} currentSeat=${snap.currentSeat} ownSeat=${snap.ownSeat} rack=${snap.ownRack.length} — Day 27`);
...
export function onServerError(error: { code: string; message: string; requestId?: string; op?: number }): void {
  console.log(`received op 102 OpServerError code=${error.code} message=${error.message} requestId=${error.requestId} op=${error.op} — Day 27`);
}
...
export function handleMatchData(opCode: number, data: Uint8Array | string): void {
  ...
  try {
    ...
    if (op === 100) {
      ...
      onServerError(payload as { code: string; message: string });
    } else if (op === 103) {
