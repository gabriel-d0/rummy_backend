...
func (m *RummyMatch) MatchLoop(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, dispatcher runtime.MatchDispatcher, tick int64, state interface{}, messages []runtime.MatchData) interface{} {
	...
	for _, msg := range messages {
		...
		// Additional melds: Playing MeldOrDiscard, only opened players, no score minimum
		if op == protocol.OpClientMeldNew {
			if err := handleMeldNew(st, senderId, env.Payload, requestId, op, dispatcher, msg, logger); err != nil {
				continue
			}
			continue
		}
		...
		// Replace joker: opened, exact tile, new meld with 2 rack tiles, atomic
		if op == protocol.OpClientReplaceJoker {
			if err := handleReplaceJoker(st, senderId, env.Payload, requestId, op, dispatcher, msg, logger); err != nil {
				continue
			}
			continue
		}

		// Other opcodes will be handled next; for now just phase validation.
